...
static PRM_Name angleName("angle", "Angle");
static PRM_Name step_sizeName("step_size", "step_size");
static PRM_Name iterationName("iteration", "Iteration");
static PRM_Name grammar_fileName("grammar_file", "grammar");
...
static PRM_Default angleDefault(45.0);
static PRM_Default step_sizeDefault(3.0);
...
SOP_Lsystem::cookMySop(OP_Context &context)
{
	...
	float step_size;
	int iteration;
	UT_String grammar_file = "";

	angle = ANGLE(now);
	step_size = STEP_SIZE(now);
	iteration = ITERATION(now);

	GRAMMAR_FILE(grammar_file, now);
	std::string gf = grammar_file.toStdString();
	...
	myplant.loadProgramFromString("F\nF->F[+F]F[-F]");  
	// myplant.loadProgram(gf);
	myplant.setDefaultAngle(angle);
	myplant.setDefaultStep(step_size);
	...
	if (boss->opStart("Building LSYSTEM"))
	{
		...
		for (int i = 0; i < branches.size(); i++) {
			// refer to SIM_SolverHair.C
			poly = GU_PrimPoly::build(gdp, 2, GU_POLY_OPEN);
			vec3 start = branches.at(i).first;
			vec3 end = branches.at(i).second;
			UT_Vector3 pos_start = UT_Vector3(start[0], start[1], start[2]);
			UT_Vector3 end_start = UT_Vector3(end[0], end[1], end[2]);
			GA_Offset ptoff_start = poly->getPointOffset(0);
			GA_Offset ptoff_end = poly->getPointOffset(1);
			gdp->setPos3(ptoff_start, pos_start);
			gdp->setPos3(ptoff_end, end_start);
		}
